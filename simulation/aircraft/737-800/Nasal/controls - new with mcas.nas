...
setlistener( "/fdm/jsbsim/fcs/MCAS-trim-enabled", MCAS_trigger_handler);
if ("/fdm/jsbsim/fcs/MCAS-trim-enabled" == 1) {
	MCAS_pilot_interrupt_handler
}

var MCAS_trigger_handler = func {
	var stab_pos = num( getprop("/fdm/jsbsim/fcs/stabilizer-pos-unit") );
	var MCAS_trim_rate = 0.27;

	# rates for
	# MCAS engaged freely
	# MCAS engaged with trim wheel attenuated by pilot hand
	# MCAS engaged but blocked by pilot (0)
	# MCAS engaged but being pushed back by pilot

	var MCAS_stab_target = stab_pos + 2.5;

	if ("/fdm/jsbsim/fcs/MCAS-trim-enabled" == 1) {
		setprop("fdm/jsbsim/fcs/stabilizer/trim-rate", MCAS_trim_rate);
		setprop("fdm/jsbsim/fcs/stabilizer/stab-target", MCAS_stab_angle );
	}
}

var MCAS_pilot_interrupt_handler = func {
	var stab_pos = num( getprop("/fdm/jsbsim/fcs/stabilizer-pos-unit") );
	var elev_cmd = num( getprop("/fdm/jsbsim/fcs/elevator-cmd-norm") );

	if ("/fdm/jsbsim/fcs/MCAS-trim-enabled" == 1 && elev_cmd < 0) {
		setprop("fdm/jsbsim/fcs/stabilizer/stab-target", stab_pos);
		setprop("/fdm/jsbsim/fcs/MCAS-trim-enabled", 0);
	}
}
